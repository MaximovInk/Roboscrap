...
namespace Anima2D
{
	public class EditorGUIExtra
	{
		...
		{
			var methodInfo = typeof(EditorGUILayout).GetMethod("SortingLayerField", BindingFlags.Static | BindingFlags.NonPublic, null, new [] { typeof(GUIContent),typeof(SerializedProperty),typeof(GUIStyle),typeof(GUIStyle) },null);
		...
		public static void MinMaxScroller(Rect position, int id, ref float value, ref float size, float visualStart, float visualEnd, float startLimit, float endLimit, GUIStyle slider, GUIStyle thumb, GUIStyle leftButton, GUIStyle rightButton, bool horiz)
		{
			...
			var minMaxScrollerMethod = editorGUIExtType.GetMethod("MinMaxScroller", BindingFlags.Static | BindingFlags.Public);

			if(minMaxScrollerMethod != null)
			{
				var parameters = new object[] { position, id, value, size, visualStart, visualEnd, startLimit, endLimit, slider, thumb, leftButton, rightButton, horiz };
		...
		public static GUIContent TempContent(string t)
		{
			...
			var tempContentMethod = editorGUIUtilityType.GetMethod("TempContent", BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic, null, new [] { typeof(string) },null);

			if(tempContentMethod != null)
			{
				var parameters = new object[] { t };
		...
		public static int GetPermanentControlID()
		{
			...
			var getPermanentControlIDMethod = guiUtilityType.GetMethod("GetPermanentControlID", BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic);
