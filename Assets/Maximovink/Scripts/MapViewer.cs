...
namespace MaximovInk
{
    public class MapViewer : MonoBehaviour
    {
        ...
        public int chunkSize => ChunkManager.Instance.rawResolution;
        ...
        public void InspectChunk(ChunkManager.ChunkSaveData data, int playerX = -1, int playerY = -1)
        {
            ...
            if (!Chunks.Any(n => n.x == data.X && n.y == data.Y))
            {
                ...
                inspectedChunk.rawData = ChunkManager.Instance.GetRawData(data.X, data.Y);
            ...
            c.texture = ReadDataFromChunk(c.rawData, c.texture, playerX, playerY);
        ...
        private void Update()
        {
            ...
            if (!FullMap.activeSelf)
            {
                ...
                InspectedChunk plCh = Chunks.FirstOrDefault(n =>
                    n.x == ChunkManager.Instance.playerChunk.SaveData.X &&
                    n.y == ChunkManager.Instance.playerChunk.SaveData.Y);
                if (plCh != null)
                {
        ...
        public Texture2D ReadDataFromChunk(bool[,] rawData, Texture2D tex = null, int playerX = -1,
            int playerY = -1)
        {
            if (tex == null)
