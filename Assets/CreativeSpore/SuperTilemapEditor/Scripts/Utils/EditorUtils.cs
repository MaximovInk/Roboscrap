...
{    
    public static class EditorUtils
    {
        ...
        {
            var idx = ArrayUtility.FindIndex(GetSortingLayerUniqueIDs(), (x) => x == sortingLayerId);
            if(idx >= 0)
            {
        ...
        {
            var idx = ArrayUtility.FindIndex(GetSortingLayerNames(), (x) => x == sortingLayerName);
            if (idx >= 0)
            {
        ...
        {
            var position = EditorGUILayout.GetControlRect(false, EditorGUIUtility.singleLineHeight, EditorStyles.popup, new GUILayoutOption[0]);
        ...
        {
            var e = Event.current;
            var sortingLayerUniqueIDs = EditorUtils.GetSortingLayerUniqueIDs();
            var sortingLayerNames = EditorUtils.GetSortingLayerNames();
            ...
            var array = new GUIContent[sortingLayerNames.Length];
            ...
            EditorUtility.SelectMenuItemFunction setEnumValueDelegate = (object userData, string[] options, int selected) =>
            {
                if (selected == options.Length - 1)
                {
                    ...
                    var tagManagerPropertyInfo = typeof(EditorApplication).GetProperty("tagManager", BindingFlags.Static | BindingFlags.NonPublic | BindingFlags.GetProperty);
                    if (tagManagerPropertyInfo != null)
                    {
                        var tagManager = (System.Object)tagManagerPropertyInfo.GetValue(typeof(EditorApplication), null);
                        var fieldInfo = tagManager.GetType().GetField("m_DefaultExpandedFoldout");
                        if (fieldInfo != null)
                        {
            ...
            var sortingLayerIdx = System.Array.IndexOf(sortingLayerUniqueIDs, layerID.intValue);
        ...
        public static bool DoToggleIconButton(string label, bool value, GUIContent icon)
        {
            ...
            var controlRect = EditorGUILayout.GetControlRect(true, 23f, new GUILayoutOption[0]);
            var position = new Rect(controlRect.xMin + EditorGUIUtility.labelWidth, controlRect.yMin, 33f, 23f);
            var content = new GUIContent(label);
            var vector = GUI.skin.label.CalcSize(content);
            var position2 = new Rect(position.xMax + 5f, controlRect.yMin + (controlRect.height - vector.y) * 0.5f, vector.x, controlRect.height);
